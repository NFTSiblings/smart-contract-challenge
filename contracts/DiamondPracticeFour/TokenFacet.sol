...
import { SaleHandlerFacetLib } from './SaleHandlerFacet.sol';

contract TokenFacet is ERC721AUpgradeable {
    function mint(uint256 amount) external payable {
        require(amount < 10, "TokenFacet: cannot mint more than 10 tokens");
        require(
            SaleHandlerFacetLib.isPrivateSaleActive() ||
            SaleHandlerFacetLib.isPublicSaleActive(),
            "TokenFacet: minting it not available now"
        );
        
        if (SaleHandlerFacetLib.isPrivateSaleActive()) {
            require(
                msg.value == amount * TokenFacetLib.getState().privateSalePrice,
                "TokenFacet: incorrect amount of Ether sent"
            );
        } else {
            require(
                msg.value == amount * TokenFacetLib.getState().publicSalePrice,
                "TokenFacet: incorrect amount of Ether sent"
            );
        }
